...
// GetLastFrame returns last frame from slice, that is returned from [*Tag.GetFrames] function.
...
func (tag *Tag) GetLastFrame(id string) Framer {
	// Avoid an allocation of slice in GetFrames if there is anyway one frame.
	if f, exists := tag.frames[id]; exists {
		return f
	}
